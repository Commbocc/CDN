...
function loadMenu() {
	...
	for (var i = 0; i < menu.length; i++) {
		if (menu[i].submenu) {
			...
			document.write("<a type=\"button\" class=\"btn btn-info dropdown-toggle\" data-toggle=\"dropdown\" href=\"/?nid="+menu[i].nid+"\">");
			document.write(menu[i].label+" <span class=\"caret\"></span></a><ul class=\"dropdown-menu\" role=\"menu\">");
			for (var si = 0; si < menu[i].submenu.length; si++) {
				document.write("<li><a href=\""+menu[i].submenu[si].nid+"\">"+menu[i].submenu[si].label+"</a></li>");
			};
			document.write("</ul></div>")
		} else {
			document.write("<a href=\"/?nid="+menu[i].nid+"\" class=\"btn btn-info\" role=\"button\">"+menu[i].label+"</a>");
		}
	};
}